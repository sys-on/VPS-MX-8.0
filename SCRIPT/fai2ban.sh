...
[[ "$apache" != "" ]] && s4="apache"
echo -e "${cor[5]} $(fun_trans "Esta é a proteção Fail2ban")"
echo -e "${cor[5]} $(fun_trans "Excelente para proteção contra ataques DDOS")"
msg -bar
if [[ ! -z "$pid_fail" ]]; then
 echo -e "${cor[2]} [1] >${cor[5]} $(fun_trans "Desinstalar Fail2ban")"
 echo -e "${cor[2]} [2] >${cor[5]} $(fun_trans "Olha para o registro")"
 msg -bar
  while [[ -z ${logxyz} || ${logxyz} != @(1|2) ]]; do
   echo -ne "\033[1;37m$(fun_trans "Selecione uma opção"): " && read logxyz
   tput cuu1 && tput dl1
  done
 case ${logxyz} in
...
echo -e "${cor[5]} $(fun_trans "Deseja Instalar  Fail2ban?")"
msg -bar
  while [[ -z ${fail2ban} || ${fail2ban} != @(s|S|n|N|y|Y) ]]; do
   echo -ne "\033[1;37m$(fun_trans "Selecione uma opção") [S/N]: " && read fail2ban
   tput cuu1 && tput dl1
  done
if [[ "$fail2ban" = @(s|S|y|Y) ]]; then
apt-get install fail2ban -y &> /dev/null
wget -O $HOME/fail2ban https://raw.githubusercontent.com/SycNET/VPS-MX-8.0/main/ArchivosUtilitarios/fail2ban-0.9.4.tar.gz &> /dev/null
...
echo -e "${cor[5]} $(fun_trans "Fail2ban estará ativo nas seguintes Portas e Serviços"):"
msg -bar
[ "$s1" != "" ] && echo -ne " $s1"
[ "$s2" != "" ] && echo -ne " $s2"
[ "$s3" != "" ] && echo -ne " $s3"
